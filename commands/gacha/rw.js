...
async function buscarImagenDelirius(tag) {
  ...
  return [...new Set(allImages)] // elimina repetidas
}
...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (me.lastRoll && now < me.lastRoll) {
      ...
      const sec = r % 60
      return m.reply(`ꕥ Debes esperar *${min}m ${sec}s* para usar *${usedPrefix + 'rw'}* de nuevo.`)
    }
    ...
    try {
      ...
      if (!mediaList.length) {
        mediaList = await buscarImagenAniList(selected.name)
      }

      if (!mediaList.length) {
        rollLocks.delete(userId)
        return m.reply(`ꕥ No se encontró imágenes para *${selected.name}*.`)
      }
      ...
      if (!chat.characters[selected.id])
      ...
      record.value = typeof globalRec.value === 'number'
        ? globalRec.value
      ...
      const owner =
        typeof record?.user === 'string' && record.user.length
