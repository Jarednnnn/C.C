...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    // Verificar si el usuario existe en los datos del chat
    const userInChat = chatUsers[userId];
    if (!userInChat) {
    ...
    const user = userInChat || {} // ya definido
    const user2 = globalUsers[userId] || {}
    
    const name = user2.name || 'Usuario'
    const birth = user2.birth || 'Sin especificar'
    const genero = user2.genre || 'Oculto'
    const comandos = user2.usedcommands || '0'
    
    // Manejo seguro de marry
    let pareja = 'Nadie';
    if (user2.marry) {
      const spouse = globalUsers[user2.marry];
      if (spouse && spouse.name) pareja = spouse.name;
    }
    ...
    // Obtener foto de perfil, asegurando que sea string
    let perfil = 'https://cdn.yuki-wabot.my.id/files/2PVh.jpeg'; // default
    try {
      const url = await client.profilePictureUrl(userId, 'image');
      if (url) perfil = url;
    } catch {
      // ya tenemos el default
    }
