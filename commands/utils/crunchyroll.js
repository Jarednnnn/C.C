import cuentasIniciales from './cuentas.js'; // importa el array de cuentas
...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (!chat.crunchyroll) {
      chat.crunchyroll = {
        accounts: [],    // Aquí se guardarán las cuentas con estado
        users: {}        // Registro de reclamos
      };
    }

    const crunchy = chat.crunchyroll;

    // Si no hay cuentas en la base de datos, cargar las iniciales desde cuentas.js
    if (crunchy.accounts.length === 0) {
      crunchy.accounts = cuentasIniciales.map(cuenta => ({
        ...cuenta,
        assigned: false,
        assignedTo: null
      }));
    ...
    // =============================================
    // PREPARAR MENSAJE CON DECORACIÓN Y AUTODESTRUCCIÓN
    // =============================================
    const expiryDate = cuenta.expiry 
