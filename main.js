...
// Función para normalizar JIDs (quitar sufijo de dispositivo y dominio)
function normalizeJid(jid) {
  if (!jid) return '';
  // Si es un objeto (ej. participante), extraer la propiedad id, jid, lid o phoneNumber
  ...
  jid = String(jid);
  // Extraer solo la parte numérica (antes de ':' y antes de '@')
...
export default async (client, m) => {
  ...
  // --- VERIFICACIÓN DE ADMINISTRADORES CON NORMALIZACIÓN ---
  if (m.isGroup) {
    try {
      groupMetadata = await client.groupMetadata(m.chat);
      groupName = groupMetadata?.subject || '';
      // Lista de objetos de administradores (con sus JIDs originales)
      const admins = groupMetadata?.participants.filter(p => p.admin === 'admin' || p.admin === 'superadmin') || [];
      
      // Normalizamos los JIDs de los administradores
      const adminsNormalized = admins.map(p => normalizeJid(p));
      // Normalizamos el sender y el bot
      const senderNormalized = normalizeJid(sender);
      const botNormalized = normalizeJid(botJid);
      
      // Comprobamos si el sender está en la lista de admins normalizados
      m.isAdmin = adminsNormalized.includes(senderNormalized);
      // Comprobamos si el bot está en la lista de admins normalizados
      isBotAdmins = adminsNormalized.includes(botNormalized);
      
      // (Opcional) Log para depurar
      // console.log('Admins normalizados:', adminsNormalized);
      // console.log('Sender normalizado:', senderNormalized, '→ es admin?', m.isAdmin);
      // console.log('Bot normalizado:', botNormalized, '→ es admin?', isBotAdmins);
    } catch (e) {
